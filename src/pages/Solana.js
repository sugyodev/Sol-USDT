...
function WealthMountain() {
  ...
  const [currentNetwork, setNetwork] = useState('Solana');
  ...
  const tvl = 45, users = 259, stake_fee = 14, collect_fee = 18, t_stakevalue = 15000, t_earncompound = 432, t_earncollect = 168, reffered_earn = 980;
  ...
  const floatIncrease = (id, val) => {
    ...
    const autoIncrease = setInterval(function () {
      ...
      if (id === 'total-staking' || id === 'referral-earn') {
        document.getElementById(id).innerHTML = '$' + curVal;
      } else if (id === 'total-collect') {
  ...
  useEffect(() => {
    floatIncrease('tvl', tvl);
    floatIncrease('users', users);
    floatIncrease('total-staking', t_stakevalue);
    floatIncrease('total-compound', t_earncompound);
    floatIncrease('total-collect', t_earncollect);
    floatIncrease('referral-earn', reffered_earn);
  }, []);
  ...
  return (
    <div className='solana'>
      <img alt="solana" src='sol-img-top.png' className='sol-top-img' />
      <img alt="solana" src='sol-img-left.png' className='sol-left-img' />
      <div className='main-content header'>
        <Container>
          <Row>
            <Col>
              <Button className="sol-custom-button mt-6 sol-switch" outline onClick={() => setSelected(!selected)}><img src="solana-icon.png" width="30" className='float-left'/>{currentNetwork}{selected?<MdExpandLess size="1.7em" className="pr-2 float-right" />:<MdExpandMore size="1.7em" className="pr-2 float-right" />}</Button>
              <div className='mt-8 py-1 sol-select-pannel' style={{ display: selected ? 'block' : 'none' }}>
                ...
                <hr />
                <Link to="/aptos"><div className='w-full cursor-pointer' onClick={() => { setNetwork('Aptos'); setSelected(!selected) }}><img src="aptos-icon.png" width="27" className='float-left' />APTOS</div></Link>
              </div>
            </Col>
            <Col className="logo">
              <img alt="solana" src='solana-logo.png' width="250" />
            </Col>
            <Col>
          ...
          <Container>
            <CardDeck className='sol-card'>
              <Card body className="text-center sol-text">
                ...
                <h5 className="source font-weight-bold text-white" id="tvl">
                  {0}
                </h5>
              </Card>
              <Card body className="text-center sol-text">
                <h5 className="calvino sol-text">Users</h5>
                <h5 className="source font-weight-bold text-white" id="users">
                  {0}
                </h5>
              </Card>
              <Card body className="text-center sol-text">
                <h5 className="calvino sol-text">Stake Fee</h5>
                <h5 className="source font-weight-bold text-white" id="stake-fee">
                  {stake_fee + '%'}
                </h5>
              </Card>
              <Card body className="text-center sol-text">
                <h5 className="calvino sol-text">Collection Fee</h5>
                <h5 className="source font-weight-bold text-white" id="collect-fee">
                  {collect_fee + '%'}
                </h5>
              </Card>
            </CardDeck>
          ...
          <div>
            <CardDeck className="p-3 sol-card">
              <Card body className="text-center sol-text">
                ...
                <h1 className="source font-weight-bold text-white" id="total-staking">{0}</h1>
                <Button outline className="sol-custom-button mt-3 source" onClick={stake}>Start a stake to see your info</Button>
              </Card>
              <Card body className="text-center sol-text">
                <h4 className="calvino sol-text">Total Earnings</h4>
                <CardDeck className='sol-card'>
                  <Card style={{ background: "transparent" }}>
                    <h4 className="source font-weight-bold text-white" id="total-compound">{0}</h4>
                  </Card>
                  <Card style={{ background: "transparent" }}>
                    <h4 className="source font-weight-bold text-white" id="total-collect">{0}</h4>
                  </Card>
                </CardDeck>
                <Row>
                  <Col>
            ...
            <CardDeck className="pl-3 pr-3 pb-3 sol-card">
              <Card body className="text-center sol-text">
                ...
                <h4 className="source font-weight-bold text-white" id="referral-earn">{0}</h4>
                <Row>
                  <Col>
      ...
      <div>
        <Container className="pt-3">
          <Card body>
            <h2 className="calvino text-center sol-text earning-calculator">Earnings Calculator</h2>
            <CardDeck className='sol-car'>
              <Card body className="text-center start-staking">
      ...
      <div className='enter-stake main-content'>
        {/* <img alt="solana" src='sub-img-bottom.png' style={{position:'absolute', width:'100%', bottom:'0px'}}/> */}
        <Container className="pt-3">
          <CardDeck className="p-3 sol-card">
            <Card body className="text-center sol-text">
              ...
              <small className="source sol-text text-left"><FaWallet size="1.7em" className="pr-2" />Your wallet: <span className="text-white font-weight-bold">0.00 BUSD</span></small>
              <small className="source sol-text text-left"><FaUserShield size="1.7em" className="pr-2" />Approved amount: <span className="text-white font-weight-bold">0.00 BUSD</span></small>
            ...
            <div>
              <Container className="pb-3 pt-3 calvino text-center">
                <CardDeck className='sol-card'>
                  ...
                  <Card className="p-3 h-full pb-6 d-6">
        ...
        <img alt="solana" src='sol-img-bottom.png' className='sol-img-bottom' />
        <img alt="solana" src='sol-img-bottom-right.png' className='sol-img-bottom-right' />
      </div>
    </div>
  )
}
